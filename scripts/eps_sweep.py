#!/usr/bin/env python3
"""BIF Hyperparameter Sweep v2 — including rmsprop_eps (preconditioner stability).

Following the Timaeus sampling guide:
  https://timaeus.co/research/2026-04-21-sampling-guide

Three phases:
  Phase 1 — Primary param sweep: lr × gamma × nbeta (fixed eps=1e-4)
  Phase 2 — Secondary param sweep: eps × lr (with promising gamma/nbeta from Phase 1)
  Phase 3 — nβ≈0 sanity check: verify gradient actually influences results

rmsprop_eps (a) controls the maximum per-parameter step size:
    ε_i(t) = lr / (sqrt(v̂_i(t)) + a)   →   max step = lr / a
  - Too large (e.g. 0.1):  preconditioner barely adjusts step sizes → like plain SGLD
  - Too small (e.g. 1e-6):  risk of division-by-zero / huge steps → instability
  - Guide recommends: 1e-3, 1e-4, 1e-5

Usage:
  python scripts/eps_sweep.py generate   # create run scripts
  bash scripts/tmux_eps_sweep.sh         # execute on 8 GPUs
  python scripts/eps_sweep.py analyze    # summarize results
"""
...
# ─── Phase 1: primary param sweep (fixed eps=1e-4) ─────────────────────────
P1_LR = [1e-6, 1e-5, 1e-4]
P1_GAMMA = [1, 10, 100, 1000]
P1_NBETA = [1, 10, 100]
P1_EPS = 1e-4  # fix eps at middle value

# ─── Phase 2: eps sweep with promising gamma/nbeta ─────────────────────────
# Use the most promising gamma/nbeta combos, sweep lr × eps
P2_LR = [1e-6, 1e-5, 1e-4]
P2_GAMMA = [10, 100, 1000]
P2_NBETA = [1, 10, 100]
P2_EPS = [1e-3, 1e-4, 1e-5]

# ─── Phase 3: nbeta≈0 sanity check ─────────────────────────────────────────
P3_LR = 1e-5
P3_GAMMA = [10, 100, 1000]
P3_NBETA = 0.0001
P3_EPS = 1e-4
...
def _fmt_eps(eps: float) -> str:
    if eps == int(eps):
        return str(int(eps))
    e = int(math.log10(eps))
    return f"1e{e}"


def _sweep_name(lr, gamma, nbeta, eps) -> str:
    return f"lr{_fmt_lr(lr)}_g{gamma}_nb{_fmt_nb(nbeta)}_eps{_fmt_eps(eps)}"


def _run_bif_cmd(lr, gamma, nbeta, eps, out_dir, experiment_name):
    return (
...
def generate():
    ...
    # ── Phase 1 ──
    for lr in P1_LR:
        for gamma in P1_GAMMA:
            for nbeta in P1_NBETA:
                name = _sweep_name(lr, gamma, nbeta, P1_EPS)
                out_dir = f"{SWEEP_BASE_DIR}/phase1/{name}"
                exp_name = f"sweep-p1-{name}"
                configs.append((
                    f"phase1/{name}",
                    _run_bif_cmd(lr, gamma, nbeta, P1_EPS, out_dir, exp_name),
    ...
    for eps in P2_EPS:
        for lr in P2_LR:
            for gamma in P2_GAMMA:
                for nbeta in P2_NBETA:
                    name = _sweep_name(lr, gamma, nbeta, eps)
                    out_dir = f"{SWEEP_BASE_DIR}/phase2/{name}"
                    exp_name = f"sweep-p2-{name}"
                    configs.append((
                        f"phase2/{name}",
                        _run_bif_cmd(lr, gamma, nbeta, eps, out_dir, exp_name),
                        _analyze_cmd(out_dir, f"{out_dir}/analysis", exp_name),
                    ))

    # ── Phase 3 ──
    for gamma in P3_GAMMA:
        name = _sweep_name(P3_LR, gamma, P3_NBETA, P3_EPS)
        out_dir = f"{SWEEP_BASE_DIR}/phase3/{name}"
        exp_name = f"sweep-p3-{name}"
        configs.append((
            f"phase3/{name}",
            _run_bif_cmd(P3_LR, gamma, P3_NBETA, P3_EPS, out_dir, exp_name),
            _analyze_cmd(out_dir, f"{out_dir}/analysis", exp_name),
        ))

    _gen_tmux_script(configs)

    n_p1 = len(P1_LR) * len(P1_GAMMA) * len(P1_NBETA)
    n_p2 = len(P2_EPS) * len(P2_LR) * len(P2_GAMMA) * len(P2_NBETA)
    n_p3 = len(P3_GAMMA)
    print(f"[gen] Phase1: {n_p1} configs (lr×gamma×nbeta, eps=1e-4)")
    print(f"[gen] Phase2: {n_p2} configs (eps×lr×gamma×nbeta)")
    print(f"[gen] Phase3: {n_p3} configs (nbeta≈0 sanity)")
    print(f"[gen] Total: {len(configs)} configs")
    print(f"[gen] Per config: {CHAINS} chains × {DRAWS} draws")
    print(f"[gen] Run: bash scripts/tmux_eps_sweep.sh")


def _gen_tmux_script(configs):
    N_GPUS = 8
    SESSION = "bif-eps-sweep"
    API_KEY = "RGvhmcyaE940jILdlzCGg"
    PROJECT = "BIFrost-eps-sweep"

    gpu_scripts_dir = ROOT / "scripts" / "eps_gpu_scripts"
    ...
    for gpu in range(N_GPUS):
        ...
        script_lines = [
            ...
            f"export SWANLAB_PROJECT={PROJECT}",
            "",
        ]

        for label, run_cmd, analyze_cmd in tasks:
            out_dir = Path(f"{SWEEP_BASE_DIR}/{label}")
            chain_npz = list(out_dir.glob("chain_*/observable_loss_trace.npz")) if out_dir.exists() else []
            has_analysis = (out_dir / "analysis" / "checkpoint_summary.csv").exists()
            if len(chain_npz) >= CHAINS and has_analysis:
                script_lines.append(f'echo "SKIP (done): {label}"')
                continue
            if len(chain_npz) >= CHAINS:
                import numpy as _np
                has_nan = any(
                    _np.isnan(_np.load(str(p))["seq_loss"]).any() for p in chain_npz
                )
                if has_nan:
                    script_lines.append(f'echo "SKIP (NaN): {label}"')
                    continue
    ...
    lines = [
        "#!/bin/bash",
        "# BIF Eps Sweep — 8 GPU tmux launcher",
        "#",
        f"# {len(configs)} configs, {N_GPUS} GPUs",
        f"# Phase1: lr×gamma×nbeta (eps=1e-4)",
        f"# Phase2: eps×lr×gamma×nbeta",
        f"# Phase3: nbeta≈0 sanity",
        "#",
        "# Usage:",
        "#   bash scripts/tmux_eps_sweep.sh          # launch",
        "#   bash scripts/tmux_eps_sweep.sh attach   # watch",
        "#   bash scripts/tmux_eps_sweep.sh kill     # stop all",
        "",
        "set -euo pipefail",
        "",
        f'SESSION="{SESSION}"',
    ...
    for gpu in range(N_GPUS):
        ...
        if not tasks:
            continue
        rel_path = f"scripts/eps_gpu_scripts/gpu{gpu}.sh"
        if gpu == 0:
        ...
        print(f"  GPU {gpu}: {len(tasks)} configs")
    ...
    script_path = SCRIPTS_DIR / "tmux_eps_sweep.sh"
    script_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    os.chmod(script_path, 0o755)
...
def analyze():
    ...
    print(f"\n{'='*100}")
    print(f"BIF Eps Sweep Results — {len(df)} chains")
    print(f"{'='*100}\n")
    ...
    if len(good) > 0:
        ...
        for _, r in good.iterrows():
            print(f"  {r['run']:50s}  loss={r['loss_mean']:.4f}  range={r['loss_range']:.4f}  snr={r['snr']:.2f}")

    csv_path = base / "eps_sweep_summary.csv"
